...
# snapshots_directory = "/home/david/datasets/seq34/results/dense_single/run_models"
snapshots_directory = "/home/david/instant-ngp/run_models"


def load_snapshot(testbed, snapshot):
	if not os.path.exists(snapshot) and snapshot in scenes:
		print('invalid snapshot path')
		exit(0)
...
def parse_args():
	...
	parser.add_argument("--mode", default="nerf", const="nerf", nargs="?", choices=["nerf", "sdf", "image", "volume"], help="Mode can be 'nerf', 'sdf', 'image' or 'volume'. Inferred from the scene if unspecified.")
...
if __name__ == "__main__":
	...
	print(os.listdir(snapshots_directory))

	snapshot_paths = [snapshots_directory + '/' + x for x in os.listdir(snapshots_directory)]
	snapshot_paths = natsorted(snapshot_paths)

	for path in snapshot_paths:
		print("Loading snapshot ", path)

		# load_snapshot(path)
		testbed.load_snapshot(path)
		psnr, minpsnr, maxpsnr, ssim = evaluate_model(args, testbed)

		model_iter = path.split('/')[-1].split('.')[0]
		# model_iter = file_name.split('_')[-1]
		print('model_iter', model_iter)
		log_file.write(f'{model_iter},{psnr},{minpsnr},{maxpsnr},{ssim}\n')
		...
		with tqdm(desc="Training", total=n_steps, unit="step") as t:
			while testbed.frame():
				...
				if testbed.training_step % SAVE_FREQ == 0:
					save_path = f'run_models/model_{testbed.training_step}.msgpack'
